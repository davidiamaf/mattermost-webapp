...
export function calculateBloom(terms) {
    return {
        bloom: Object.values(terms).filter(({Text}) => Text).map(({Text}) => crypto.createHash('sha256').update(Text.toLowerCase()).digest()).reduce((prev, current) => buffer.or(current, prev), Buffer.from([0])),
        terms: new Map(Object.entries(terms).map(([key, value]) => [key, {key, ...value}])),
    };
}

store.acronymBloom = calculateBloom({
    mdi: {
        Text: 'MDI',
